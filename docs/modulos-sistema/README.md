...
| `13 - dashboard` | **Orquestrador genérico** de dashboards: catálogo de tipos em código (`enum TipoDashboard`, hoje só `FINANCEIRO`, mesmo padrão de catálogo dos documentos `02`/`03`), substituição da tela inicial pós-login por `/dashboard`, seletor de tipo exibido só a partir do 2º tipo cadastrado (URL `?tipo=`). Não introduz tabela nova; não define nenhuma permissão — exige apenas autenticação. Nenhum card ou cálculo financeiro vive aqui, delegado ao dashboard concreto ativo | `Em análise` |
| `13a - dashboard-financeiro` | Primeiro dashboard concreto, plugado no `13` como tipo `FINANCEIRO`. Tela somente leitura com 8 cards sobre dados já existentes (`CONTAS`, `RECEITAS`, `DESPESAS`, `DESPESAS_USUARIO`, `CATEGORIAS`, `CARTOES_CREDITO`, `CONTATOS`): saldo consolidado, saldo por conta, receitas x despesas do mês, despesas por categoria, pagas x pendentes, total rateado por pessoa, limite usado x disponível por cartão (resolve a Observação 11 do `07`) e evolução anual/mensal com granularidade condicional (precedente: RN30 do `09`). Filtro global de competência (`mes_atual - 1`) com contrato uniforme de parâmetro; sem AJAX; gráficos ApexCharts embutidos pelo servidor. Escopo row-level, inclusive para o ADMIN. Nova permissão `DASHBOARD_VISUALIZAR`. Não introduz tabela nova | `Em análise` |
