...
- o **vínculo opcional** com uma conta de débito, escolhida entre as contas do próprio usuário (combobox alimentado pelo documento `2.03 - manter-conta`);
- o **contrato de consulta** que a tela de Despesa (documento `2.06`) usa para carregar os cartões ativos do usuário no combobox de cartão.
...
- Definição das permissões atômicas que esta tela usa (`CARTOES_LISTAR`, `CARTOES_INSERIR`, `CARTOES_EDITAR`, `CARTOES_EXCLUIR`, `CARTOES_DESATIVAR`). O RBAC completo é do Documento 0 e a tela que edita perfil × permissão é o documento `5.02 - manter-perfil-permissao`.

**Não contempla:**
- **Fatura de cartão** — ciclo de vida (`ABERTA` → `FECHADA` → `PAGA` / `PAGA_PARCIAL`), fechamento, pagamento e o valor parcial da fatura em aberto. Tudo isso é o documento `2.08 - manter-fatura-cartao`. Aqui, apenas os campos `CACR_DIA_FECHAMENTO` e `CACR_DIA_VENCIMENTO`, que a geração da fatura do documento `2.08` consome.
- CRUD de Despesa, que **consome** o cartão (`CACR_ID`) — documento `2.06`.
- CRUD de Conta, que fornece o combobox de conta de débito — documento `2.03`.
- Alerta de estouro de limite, projeção de fatura e "limite disponível" — cálculos do Dashboard (documento `1.02`) e da fatura (documento `2.08`). Ver Seção 17.
...
| 2 | **Cartão é dado do usuário, não catálogo.** `CARTOES_CREDITO` tem `USU_ID` obrigatório. Não há "cartão do sistema" nem visão administrativa global. Todo endpoint filtra pelo `USU_ID` do autenticado no serviço ([RN02](#rn02)); o id da requisição nunca amplia o escopo. Modelo igual ao do documento `2.03 - manter-conta`. | [RN02](#rn02), [RNF06](#rnf06) |
...
| 5 | **Combobox de conta vem do documento `2.03`.** O modal de cartão carrega as contas ativas do usuário pelo endpoint de opções de contas do documento `2.03 - manter-conta`. Este documento não redefine esse contrato — apenas o consome ([SB02](#sb02)). | Documento `2.03 - manter-conta` |
...
| 9 | **Exclusão travada por vínculo.** Excluir um cartão referenciado por alguma fatura (`FATURAS_CARTAO`) ou por alguma despesa (`DESPESAS`) não excluída é **bloqueado** ([RN08](#rn08)); a alternativa é **desativar** o cartão. Mesma lógica da [RN06 do documento `2.01 - manter-categoria`](../2.01%20-%20manter-categoria/documento-analise-manter-categoria.md#rn06). O comportamento sobre despesas é parametrizável ([Seção 12](#12-parâmetros-de-sistema)); sobre faturas o bloqueio é duro, porque `FATURAS_CARTAO.CACR_ID` é `NOT NULL`. | [RN08](#rn08), [QUADRO_DESCRITIVO_8](../../00%20-%20analise-geral/documento-0-fundacao.md#quadro-descritivo-8) |
...
| 11 | **Limite é informativo nesta versão.** `CACR_LIMITE` é guardado e exibido, mas a tela não calcula "limite disponível" nem alerta quando as despesas do ciclo passam do limite — isso depende da fatura (documento `2.08`) e do Dashboard (documento `1.02`). Ver Seção 17. | Seção 17 |
...
| <a id="rf09"></a>RF09 | O sistema deve fornecer à tela de Despesa (documento `2.06`) a lista dos cartões **ativos do usuário autenticado**, para o combobox de cartão. | Alta | Em análise |
...
| <a id="caus06"></a>CAUS06 | Selecionar Cartão numa Despesa | [PERF01](#perf01), [PERF02](#perf02) | Ao cadastrar uma despesa no cartão, o usuário escolhe o cartão num combobox alimentado pelos seus cartões ativos. Contexto — regra no documento `2.06`. ([RF09](#rf09)) |
...
| `CONTAS` | Documento 0 — [QUADRO_DESCRITIVO_5](../../00%20-%20analise-geral/documento-0-fundacao.md#quadro-descritivo-5) | Somente leitura — combobox de conta de débito ([SB02](#sb02)), restrito às contas ativas do usuário. Contrato no documento `2.03` |
...
| <a id="sb02"></a>SB02 | CONTA DE DÉBITO | Itens carregados das **contas ativas do usuário autenticado** pelo endpoint de opções de contas do documento `2.03 - manter-conta`, ordenados por descrição. No modal, a opção "Nenhuma" é adicional (conta de débito é opcional — [RN07](#rn07)). Nunca `<option>` fixo no HTML. |
...
Todos os endpoints deste documento resolvem o `USU_ID` do usuário autenticado a partir do contexto de segurança e aplicam o escopo por usuário no serviço ([RN02](#rn02)). O combobox de conta de débito ([SB02](#sb02)) consome o endpoint de opções de contas do documento `2.03 - manter-conta` — não reproduzido aqui.
...
| Retorna os cartões **ativos do usuário autenticado** para o combobox da tela de Despesa (documento `2.06`). Executa [C2](#c2). Campos: id, descricao, bandeira, finalCartao, diaFechamento, diaVencimento. Exige apenas usuário autenticado — o escopo por `USU_ID` já restringe o resultado. | | | | |
...
| <a id="rn12"></a>RN12 | `CACR_LIMITE` é um dado informativo nesta versão. O serviço não valida o valor das despesas do ciclo contra o limite, não calcula "limite disponível" e não emite alerta de estouro — isso depende da fatura (documento `2.08`) e do Dashboard (documento `1.02`). Ver a decisão em aberto na Seção 17. |
...
1. Usuário clica em "Novo cartão" → modal QUADRO_DESCRITIVO_3.
        Conta de débito ← SB02 (contas ativas do usuário, documento 2.02).
...
- Conta de débito **opcional** (`CTA_ID` nullable no Documento 0); quando informada, tem de ser conta ativa do próprio usuário. Combobox alimentado pelo documento `2.03`.
- Trava de exclusão: cartão com faturas ou despesas vinculadas → bloqueia, oferece desativar (espelha a [RN06 do documento `2.01`](../2.01%20-%20manter-categoria/documento-analise-manter-categoria.md#rn06)). Bloqueio por faturas é duro (`FATURAS_CARTAO.CACR_ID` é `NOT NULL`); por despesas, parametrizável (`CARTAO_EXCLUSAO_BLOQUEIA_EM_USO`).
- [EDP07](#edp07) (`/cartoes/opcoes`) fornece os cartões ativos do usuário para a tela de Despesa (documento `2.06`), exigindo só autenticação.
- Fatura de cartão (ciclo de vida, fechamento, pagamento, valor parcial) fica **inteiramente** no documento `2.08`. Aqui só o cadastro do cartão e os dois dias que a geração da fatura consome.
...
- `CACR_LIMITE`: fica só informativo (decisão atual — [RN12](#rn12)), ou a tela / o Dashboard passam a alertar quando as despesas do ciclo ultrapassam o limite? (provável fora de escopo — trataria no documento `2.08` ou `1.02`.)
- Exibir "fatura atual" / "limite disponível" nesta tela (read-only, calculado a partir das despesas do ciclo), ou deixar 100% para o documento `2.08`?
...
- Documento `2.01 - manter-categoria`: `../2.01 - manter-categoria/documento-analise-manter-categoria.md` (trava de exclusão com oferta de desativar espelhada aqui — RN06 → [RN08](#rn08)).
- Documento `2.03 - manter-conta`: `../2.03 - manter-conta/documento-analise-manter-conta.md` (modelo de dado-do-usuário e endpoint de opções de contas consumido por [SB02](#sb02)) — em elaboração em paralelo.
- Documento `2.06 - manter-despesa`: `../2.06 - manter-despesa/documento-analise-manter-despesa.md` (consome [EDP07](#edp07); a compra no cartão é `Despesa` com `CACR_ID`).
- Documento `2.08 - manter-fatura-cartao`: `../2.08 - manter-fatura-cartao/documento-analise-manter-fatura-cartao.md` (ciclo de vida e pagamento da fatura; consome `CACR_DIA_FECHAMENTO` / `CACR_DIA_VENCIMENTO`).
