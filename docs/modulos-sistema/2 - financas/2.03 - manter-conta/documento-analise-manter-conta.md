...
**Não contempla:**
- CRUD de Receita, Despesa, Transação Bancária, Cartão de Crédito e Investimento, que **consomem** a conta — documentos `2.04` a `3.01`.
- CRUD de **Instituição Financeira** (`INSTITUICOES_FINANCEIRAS`) — documento `2.02 - manter-instituicao-financeira`. Aqui, a instituição é apenas escolhida num combobox alimentado pelo endpoint daquele documento.
- Conexão de contas via Open Finance, consentimento e sincronização de saldo/transações (que preenche `CTA_SALDO_SINCRONIZADO_EM`) — documentos `2.09` e `2.10`.
- Cálculo do **saldo geral consolidado** e o efeito de `CTA_FL_CONSIDERA_SALDO` no painel — documento `1.02 - dashboard-financeiro`.
...
| 5 | **O saldo é manual nesta tela.** `CTA_SALDO` é informado no cadastro (campo "Saldo inicial") e alterado depois por uma **ação dedicada "Ajustar saldo"** ([RT08](#rt08) / [EDP08](#edp08)), não pela edição cadastral. A sincronização via Open Finance, que preenche `CTA_SALDO_SINCRONIZADO_EM`, é dos documentos `2.09`/`2.10`. | [RN10](#rn10) |
...
| 9 | **`CTA_FL_CONSIDERA_SALDO`** controla se o `CTA_SALDO` da conta entra no "saldo geral consolidado" do Dashboard (documento `1.02`). Default `TRUE`. Aqui só se mantém o flag; o efeito no painel é do documento `1.02`. | [RN09](#rn09) |
| 10 | **Instituição vem do documento `2.02`.** O combobox de instituição é alimentado pelo endpoint de opções de `INSTITUICOES_FINANCEIRAS` do documento `2.02 - manter-instituicao-financeira` — análogo ao [EDP07 do documento `2.01`](../2.01%20-%20manter-categoria/documento-analise-manter-categoria.md#edp07). Contrato assumido: `GET /instituicoes-financeiras/opcoes`, devolvendo id, nome, tipo e situação. | [SB02](#sb02) |
...
| 13 | **Sem carga inicial.** `CONTAS` nasce vazia. As contas surgem do uso — cadastro manual nesta tela ou *backfill* do Open Finance (documentos `2.09`/`2.10`). | Documento 0, Seção 6.4 |
| 14 | **Moeda diferente de BRL.** A v1.0 apenas armazena `CTA_MOEDA` (ISO 4217, default `BRL`). Não há conversão; o tratamento de contas não-BRL no saldo consolidado é decisão do documento `1.02` (ver Seção 17). | [RN05](#rn05) |
| 15 | **Contas conectadas ao Open Finance** (`CTA_SALDO_SINCRONIZADO_EM` não nulo) aparecem nesta tela, mas a gestão do vínculo e a sincronização são dos documentos `2.09`/`2.10`. Se esta tela deve tratar essas contas como somente-leitura é ponto em aberto na Seção 17. | [RN11](#rn11) |
...
| `INSTITUICOES_FINANCEIRAS` | Documento 0 — [QUADRO_DESCRITIVO_4](../../00%20-%20analise-geral/documento-0-fundacao.md#quadro-descritivo-4) | Somente leitura (combobox de instituição — endpoint do documento `2.02`) |
...
Nenhuma. `CONTAS` nasce vazia (Documento 0, Seção 6.4 — grupo 3, sem *seed*). As contas surgem do cadastro manual nesta tela ou do *backfill* do Open Finance (documentos `2.09`/`2.10`).
...
| <a id="rt10"></a>RT10 | Ao abrir o modal de conta ([QUADRO_DESCRITIVO_3](#quadro-descritivo-3)), carregar o combobox Instituição pelo endpoint de opções do documento `2.02` ([SB02](#sb02)) e o combobox Moeda pela lista curada do front ([SB03](#sb03)). Nunca renderizar `<option>` fixo no HTML. |
...
> O combobox de instituição do modal de conta e do filtro é alimentado pelo endpoint de opções de `INSTITUICOES_FINANCEIRAS` definido no documento `2.02 - manter-instituicao-financeira` (contrato assumido: `GET /instituicoes-financeiras/opcoes`) — este documento não define esse endpoint.
...
| <a id="rn05"></a>RN05 | `CTA_MOEDA` é obrigatória, `CHAR(3)` no padrão ISO 4217, default `BRL`. A v1.0 apenas armazena a moeda; não há conversão de valores. O tratamento de contas não-BRL no saldo consolidado é do documento `1.02` (ver Seção 17). |
...
| <a id="rn09"></a>RN09 | `CTA_FL_CONSIDERA_SALDO` (default `TRUE`) indica se o `CTA_SALDO` da conta entra no "saldo geral consolidado" do Dashboard. Esta tela apenas mantém o flag; o cálculo do saldo consolidado e o efeito exato do flag são do documento `1.02`. |
...
| <a id="rn11"></a>RN11 | Contas com `CTA_SALDO_SINCRONIZADO_EM` não nulo têm o saldo mantido pela sincronização de Open Finance (documentos `2.09`/`2.10`). A v1.0 permite o ajuste manual e a edição cadastral dessas contas nesta tela; se a gestão deve ser exclusiva dos documentos `2.09`/`2.10` (tela somente-leitura aqui) é ponto em aberto na Seção 17. |
...
E o efeito no saldo consolidado é validado no documento `1.02`.
...
Descrição: Levantamento a partir do Documento 0 (Observação 11; [QUADRO_DESCRITIVO_5](../../00%20-%20analise-geral/documento-0-fundacao.md#quadro-descritivo-5)), do CRUD de `InstituicaoFinanceiraUsuario` da geração 1 (`dsc-backend`) e do padrão de tela de catálogo do documento `2.01 - manter-categoria`.
...
- O combobox de instituição é alimentado pelo endpoint de opções do documento `2.02 - manter-instituicao-financeira`.
- Exclusão lógica; trava "conta em uso" espelha a RN06 do documento `2.01`, parametrizável por `CONTA_EXCLUSAO_BLOQUEIA_EM_USO`. Conta com transação bancária nunca é excluível (FK `NOT NULL`).
...
- **`CTA_FL_CONSIDERA_SALDO`:** confirmar com o documento `1.02` o efeito exato no "saldo geral consolidado" e se o default `TRUE` é o desejado.
...
- **Contas conectadas ao Open Finance** (`CTA_SALDO_SINCRONIZADO_EM` não nulo): esta tela permite editar/ajustar saldo/excluir essas contas, ou elas ficam somente-leitura aqui e são geridas nos documentos `2.09`/`2.10`?
...
- Documento `2.01 - manter-categoria`: `../2.01 - manter-categoria/documento-analise-manter-categoria.md` (padrão de tela, trava "em uso" espelhada aqui na conta, forma e voz).
- Documento `2.02 - manter-instituicao-financeira`: `../2.02 - manter-instituicao-financeira/` (a escrever) — fonte do endpoint de opções de instituição.
