...
**Não contempla:**
- CRUD de **Cartão de Crédito** (`CARTOES_CREDITO`) — documento `2.04 - manter-cartao-credito`.
- CRUD de **Despesas** (`DESPESAS`) — documento `2.06 - manter-despesa`. As despesas no cartão são criadas e editadas naquele módulo.
- CRUD de **Contas** (`CONTAS`) — documento `2.03 - manter-conta`.
- Sincronização direta de faturas do provedor Open Finance — documento `2.09`.
...
| ID | NOME | DESCRIÇÃO |
|---|---|---|
| <a id="sb01"></a>SB01 | CARTÃO DE CRÉDITO | Lista dos cartões de crédito ativos pertencentes ao usuário autenticado (`GET /cartoes/opcoes` do documento `2.04`). |
| <a id="sb02"></a>SB02 | CONTA DE DÉBITO | Lista das contas bancárias ativas pertencentes ao usuário autenticado (`GET /contas/opcoes` do documento `2.03`). |
...
- Documento `2.04 - manter-cartao-credito` — Especificação do CRUD de cartões de crédito e endpoint `GET /cartoes/opcoes`.
- Documento `2.07 - manter-transacao-bancaria` — Especificação de transações bancárias e conciliação de pagamentos de fatura.
