#!/usr/bin/env python3
# Diagramas draw.io do documento 2.02 - Manter Instituicao Financeira.
...
M.parseString(xml1)
open(os.path.join(OUT, "manter-instituicao-financeira-prototipo.drawio"), "w", encoding="utf-8").write(xml1)
...
d.rect(560, 490, 360, 150,
       ...
       "(id externo + provedor → INFI_ID). Documento 2.09 é consumidor.\n\n"
...
M.parseString(xml2)
open(os.path.join(OUT, "manter-instituicao-financeira-der.drawio"), "w", encoding="utf-8").write(xml2)
...
M.parseString(xml3)
open(os.path.join(OUT, "manter-instituicao-financeira-casos-uso.drawio"), "w", encoding="utf-8").write(xml3)
