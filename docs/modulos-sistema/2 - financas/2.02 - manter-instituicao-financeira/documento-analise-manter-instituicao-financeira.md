...
Na geração 1 (API REST + SPA Angular), a instituição financeira é a entidade `InstituicaoFinanceira`, exposta por um `InstituicaoFinanceiraController` REST cru (`GET /instituicoes-financeiras`, `POST /inserir`, `PUT /editar/{id}`, `DELETE /excluir/{id}`), **sem tela** no Angular. O vínculo "conta do usuário nesta instituição" era a entidade `InstituicaoFinanceiraUsuario`. O Documento 0 (Observação 11) travou a decisão de renomear `INSTITUICOES_FINANCEIRAS_USUARIO` para **`CONTAS`** (documento `2.03`), mantendo `INSTITUICOES_FINANCEIRAS` como o **catálogo global de bancos e corretoras** — este documento.
...
- Definição das permissões que estas telas usam. O RBAC completo é do Documento 0 e a tela que edita perfil × permissão é o documento `5.02 - manter-perfil-permissao`.

**Não contempla:**
- CRUD de **Conta**, que representa a conta do usuário numa instituição — documento `2.03`. `CONTAS` **consome** o catálogo por FK `INFI_ID`.
- CRUD de **Cartão de Crédito** (`2.04`) e **Investimento** (`3.01`), que também referenciam a instituição.
- O job de sincronização de Open Finance e o passo de importação/conciliação de contas — documentos `2.09` e `2.10`. Aqui, apenas o mapa que a importação consulta para resolver a instituição de uma conta trazida do provedor.
...
| 8 | **Mapa de provedor em tela própria.** `OPFI_INSTITUICAO_PROVEDOR` é mantido numa **tela administrativa dedicada**, não numa aba do modal de instituição (ver Seção 17). Como `OPFI_PROVEDORES` vem de carga inicial, esta tela funciona antes das telas de Open Finance (`2.09`/`2.10`). É a análoga da tela "Categorias por Provedor" do documento `2.01`. | [QUADRO_DESCRITIVO_4](#quadro-descritivo-4) |
| 9 | **Uso do mapa na importação de contas.** Quando uma conexão a um provedor devolve as contas do usuário, cada conta externa vem com o id do *connector*/instituição no provedor. O passo de importação (documento `2.10`) casa esse id + o provedor da conexão com uma linha de `OPFI_INSTITUICAO_PROVEDOR` e resolve o `INFI_ID`. Sem correspondência, a conta importada fica sem instituição do catálogo até a conciliação manual. Este documento só mantém o mapa; a regra de casamento é detalhada no documento `2.10`. | [RN14](#rn14) |
...
| <a id="rf13"></a>RF13 | O sistema deve, no passo de importação das contas de Open Finance, consultar o mapa instituição × provedor para resolver a instituição de uma conta trazida do provedor (regra detalhada no documento `2.10`). | Média | Em análise |
...
| <a id="caus10"></a>CAUS10 | Resolver Instituição de Conta Importada | Sistema (job de importação) | O passo de importação de contas de Open Finance consulta o mapa para resolver a instituição de uma conta trazida do provedor. Contexto — regra no documento `2.10`. ([RF13](#rf13)) |
...
- Instituições são um **catálogo global** e geridas só por ADMIN — `INSTITUICOES_FINANCEIRAS` não tem `USU_ID` no Documento 0. O vínculo usuário ↔ instituição da geração 1 (`InstituicaoFinanceiraUsuario`) virou a tabela `CONTAS` (documento `2.03`).
...
- **Mapa de provedor:** confirmar a tela própria (adotada, como no documento `2.01`) em vez de uma aba no modal de instituição.
...
- Documento `5.02 - manter-perfil-permissao`: `../../5 - administracao/5.02 - manter-perfil-permissao/documento-analise-manter-perfil-permissao.md` (catálogo e governança de permissões granulares por operação).
- Documento `2.01 - manter-categoria`: `../2.01 - manter-categoria/documento-analise-manter-categoria.md` (documento-espelho — catálogo administrativo global + tela de mapa de provedor).
