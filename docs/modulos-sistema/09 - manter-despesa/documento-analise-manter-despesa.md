...
| <a id="qdd1-17"></a>17 | STATUS DE PAGAMENTO | Tipo: Coluna (badge)<br>Ordenação: Sim | "Pago" (verde) quando [C1](#c1).statusPagamento = `SIM`; "Em aberto" (amarelo) quando `NAO`; "N/A" (cinza) quando `NAO_SE_APLICA`. "Excluída" quando há `audit_data_exclusao`. |
...
> OBSERVAÇÕES: Acionado pelo ícone "Registrar pagamento" do grid ([ID23](#qdd1-23)), restrito a [PERM05](#perm05) (`DESPESAS_PAGAR`). Marca `DESP_IND_STATUS_PAGAMENTO = SIM` e grava `DESP_DT_PAGAMENTO`. Oculto para despesas já pagas ou com status `NAO_SE_APLICA` (exceto despesas importadas do cartão com status pendente).
...
| <a id="rn21"></a>RN21 | **Baixa de pagamento.** [EDP07](#edp07) (individual) e [EDP08](#edp08) (lote) marcam `DESP_IND_STATUS_PAGAMENTO = SIM` e gravam `DESP_DT_PAGAMENTO` (data informada; no lote, a mesma para todas). Aplica-se a despesas com status `NAO` e a despesas importadas do cartão (`DESP_ORIGEM = 'IMPORTACAO'`) com status pendente — despesa já `SIM` é ignorada (idempotente); despesa manual de cartão com `NAO_SE_APLICA` é recusada → [MSG20](#msg20). Numa série parcelada, a baixa é **por parcela** — não há "pagar a série toda" na v1.0. Espelha `pagarDespesa` / `pagarDespesas` da geração 1. |
...
| <a id="rn23"></a>RN23 | **Compra no cartão — fronteira com a fatura.** Quando `CACR_ID` está preenchido e a origem é `MANUAL`, o serviço fixa `DESP_MEIO_PAGAMENTO = CREDITO` e `DESP_IND_STATUS_PAGAMENTO = NAO_SE_APLICA` (parâmetro `DESPESA_COMPRA_CARTAO_STATUS_NAO_SE_APLICA`), e a ação "Registrar pagamento" não se aplica ([RT11](#rt11)). Para despesas importadas do cartão (`DESP_ORIGEM = 'IMPORTACAO'`), o status inicial é `NAO` (Em aberto) e o registro de pagamento (individual e em lote) é permitido normalmente. O **vínculo com a fatura** (`DESP.FTCA_ID`), o fechamento e o cálculo do total da fatura são do documento `11 - manter-fatura-cartao` (Documento 0, Observação 15) — nesta tela `FTCA_ID` fica nulo. O pagamento da fatura em si é uma `TransacaoBancaria` com `TRBA_FL_PAGAMENTO_FATURA` (documento `10`, Documento 0, Observação 16). |
...
3. Usuário confirma → EDP08.
        ...
        ├─ Alguma é compra no cartão manual (status NAO_SE_APLICA)  → ignorada (RN21), as demais seguem (despesas importadas com status pendente são baixadas).
