...
| <a id="qdd1-8a"></a>8a | NÚMERO DE ORDEM | Tipo: Coluna (`#` / `Nº Ordem`) com alça de arrasto (`drag handle`)<br>Ordenação: Sim (numérica) | Exibe o número sequencial da linha (1, 2, 3...) derivado inicialmente da ordenação padrão vigente ([RT21](#rt21)). Permite ordenação explícita crescente/decrescente pelo cabeçalho da coluna e reordenação interativa das linhas via Drag and Drop (arrastar e soltar), recalculando a numeração sequencial e persistindo a ordem personalizada em armazenamento local ([RT22](#rt22)). |
...
| <a id="rt22"></a>RT22 | **Reordenação por Drag and Drop e Coluna Número de Ordem:** O grid de Despesas disponibiliza a coluna **Nº Ordem** ([ID8a](#qdd1-8a)), contendo um identificador ordinal (1..N) e uma alça de arrasto (`ph-dots-six-vertical`). Ao carregar os dados, a numeração sequencial é atribuída inicialmente com base na ordenação padrão vigente ([RT21](#rt21)) ou na ordem personalizada salva no navegador (`localStorage`). O usuário pode arrastar qualquer linha para uma nova posição (Drag and Drop nativo HTML5). Ao soltar a linha: (1) a nova sequência de itens é consolidada; (2) os números ordinais de todas as linhas são recalculados sequencialmente (1..N); (3) a ordem resultante é persistida no `localStorage` por competência/usuário; (4) a ordenação do grid é fixada na coluna `ordem` ascendente; (5) a visualização da tabela é re-renderizada imediatamente preservando seleções e estado visual. Ao clicar no cabeçalho "Nº Ordem", a ordenação numérica é alternada entre crescente e decrescente. |
