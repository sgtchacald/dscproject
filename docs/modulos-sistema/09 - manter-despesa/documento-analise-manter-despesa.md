...
| <a id="qdd3-23"></a>23 | LISTA DE CO-PARTICIPANTES E MINHA COTA | Tipo: Tabela editável e painel interativo<br>Colunas: Contato (suggestion box / atalho inline), Valor da fatia (monetário com atalho 100%), Status de acerto (checkbox/badge), Remover | Cada linha grava uma `DESPESAS_USUARIO` (`CONT_ID`). Contato via [SB06](#sb06) com atalho para cadastro rápido ([EDP11](#edp11)). Soma das fatias validada por [RT10](#rt10) e [RN17](#rn17). Inclui controle interativo "Minha Cota (Você - Titular)" com campo monetário editável bidirecional, botão "Zerar minha cota" (atribuindo o total ao co-participante) e botão "100% / Total" em cada linha para rateio integral rápido (cota do titular = `R$ 0,00`). |
...
| <a id="rt10"></a>RT10 | A seção Rateio ([ID22](#qdd3-22)) só é renderizada quando o usuário tem [PERM07](#perm07) (`DESPESA_RATEAR_MULTIUSUARIO`). Ao adicionar uma linha, o campo Contato usa [SB06](#sb06), exibindo o nome, dados de contato e badge `Externo` ou `Usuário do Sistema`. O usuário pode clicar em "+ Novo Contato Externo" para abrir um submodal compacto (Nome obrigatório, Telefone e Chave Pix opcionais), cadastrar via [EDP11](#edp11) e vinculá-lo imediatamente à nova fatia. O rateio disponibiliza: (1) campo interativo e editável "Minha Cota (Você - Titular)" com máscara monetária contínua (`pt-BR`, `R$ 0,00`); (2) botão de atalho "Zerar minha cota", que define a cota do titular em `R$ 0,00` e atribui o saldo integral aos co-participantes; (3) botão de atalho "100% / Total" na linha de cada co-participante para atribuir instantaneamente o valor integral/saldo àquele contato; (4) divisão automática equitativa ao adicionar contatos quando o valor da despesa já estiver preenchido (entre Titular e Contatos caso a cota não esteja zerada, ou entre os Contatos caso a cota seja `R$ 0,00`); (5) botão dedicado "Dividir igualmente" para redistribuir equitativamente o total da despesa com um clique a qualquer momento, alocando resíduos de centavos para garantir que a soma feche perfeitamente no total da despesa; (6) discriminativo de parcelas nas fatias da tabela e em Minha Cota (`Nx de R$ Y,YY`) quando a despesa for parcelada ([RN18](#rn18)), sincronizado com a pré-visualização da parcela ([RT09](#rt09)); (7) sincronização bidirecional em tempo real: a cada alteração na fatia de um contato, Minha Cota é recalculada (`Valor − soma das fatias`), e caso a Minha Cota seja editada manualmente (com 1 contato no rateio), a fatia do contato assume a diferença (`Valor − Minha Cota`). É expressamente permitido o rateio com valor zerado para o titular (`Minha cota = R$ 0,00`), hipótese em que 100% da despesa é absorvida pelos co-participantes ([RN17](#rn17)). Se a soma das fatias exceder o Valor ([MSG17](#msg17)), o sistema exibe alerta em vermelho e bloqueia o salvamento. O acerto de uma fatia (marcar como acertada + data) é editável na própria linha e persistido junto com o Salvar, ou pelo ícone "Ratear" do grid ([ID23](#qdd1-23)) que reabre o modal de edição focando e rolando suavemente até a seção de Rateio com foco no campo de busca de contato. |
