...
- **O histórico de revisões está no escopo:** a tela mostra, para cada parâmetro, quem mudou, quando, o valor anterior → o novo e o motivo (via Hibernate Envers, como o `/usuarios/historico/{id}` do documento `5.01 - manter-usuario`). É o objetivo declarado da tela — controlar os valores editados ao longo do tempo.

Este é um documento de **infraestrutura**, na posição `5.03` (logo após o RBAC, antes dos catálogos de domínio), porque os documentos de tela seguintes (`2.01` a `2.04`) já declaram parâmetros na própria Seção 12 que passam a ser linhas geridas por aqui.
...
| 2 | **O catálogo de parâmetros vive no código.** Uma interface `ParametroDefinido` (`codigo`, `nome`, `descricao`, `modulo`, `tipo`, `valorDefault`), um `enum` por módulo que a implementa (ex.: `ParametrosCategoriaCatalogo`, `ParametrosContaCatalogo`), e um `ParametroCatalogoService` que estende o sincronizador genérico `SincronizadorDeCatalogo<D, L>` (pacote `catalogo/`). Os enums são descobertos por varredura do pacote (`DescobridorDeCatalogo`) — sem agregador escrito à mão. Mesmo desenho do catálogo de permissões (documento `5.02 - manter-perfil-permissao` — `PermissaoDefinida` + `PermissaoCatalogoService`). | [RN05](#rn05), [RN06](#rn06) |
...
| 6 | **Histórico de revisões no escopo.** A tela tem uma visão do histórico de um parâmetro (Envers / `REVINFO`): quando, quem, `valor` anterior → novo e o motivo. Espelha o `/usuarios/historico/{id}` do documento `5.01 - manter-usuario`. É o objetivo declarado da tela. | [RF06](#rf06), [EDP05](#edp05) |
...
> OBSERVAÇÕES: Somente leitura. Uma linha por revisão do parâmetro (Hibernate Envers), da mais recente para a mais antiga. Espelha o histórico do documento `5.01 - manter-usuario`.
...
`USER` não recebe nenhuma permissão deste módulo — o menu 'Parâmetros Globais' e todos os endpoints ficam indisponíveis. Um perfil "auditor de configuração" (só `PARAMETROS_LISTAR`) é uma configuração possível que o editor de perfil (documento `5.02 - manter-perfil-permissao`) permite montar.
...
Descrição: Levantamento a partir da engenharia reversa da feature `ParametroGlobal` do `portal-lgpd-api` (`models/entities/ParametroGlobal.java`, `dtos/validation/ParametroGlobalValidator.java`, `services/ParametroGlobalService.java` e os scripts `sql/executar/executados/*carga-parametro-global*`), do padrão de catálogo sincronizado do documento `5.02 - manter-perfil-permissao` e do histórico Envers do documento `5.01 - manter-usuario`.

**Decisões tomadas:**
- A tela **não cria nem exclui** parâmetro (diferente da geração 1). Os parâmetros são declarados num **catálogo no código** (`ParametroDefinido` + um `enum` por módulo, descobertos por varredura do pacote; `ParametroCatalogoService` estende o `SincronizadorDeCatalogo<D, L>` genérico), semeados na inicialização por um sincronizador que **nunca apaga** — parâmetro que sumiu do código vira órfão (`PAGL_FL_ORFA`). Justificativa: parâmetro é contrato da aplicação (o código lê `buscarValorPorCodigo`), não dado editável livre.
...
- **Histórico de revisões no escopo** (não adiado) — visão de revisões via Envers, espelhando o `/usuarios/historico/{id}` do documento `5.01`.
...
- Caminho de navegação: "Administração > Parâmetros Globais". Posição `5.03` — tela de infraestrutura, logo após o RBAC.
...
**A Confirmar:**
- **Parâmetro órfão** ([RN06](#rn06)): a tela mostra e **deixa editar** o valor (pode haver rotina antiga ainda lendo), ou **trava** a edição e retorna [MSG09](#msg09), como a permissão órfã do documento `5.02`?
- **Consolidação das Seções 12 dos documentos de tela:** os parâmetros que os documentos `2.01`/`2.02`/`2.03`/`2.04` declaram nas próprias Seções 12 passam a ser linhas geridas aqui — confirmar que a Seção 12 de cada documento de tela é a fonte que o catálogo do código consolida (uma classe/`enum` de catálogo por módulo).
- **Gatilho manual de sincronização** pela tela: expor um botão "Sincronizar catálogo" (como o do documento `5.02`) exigiria uma terceira permissão ou reuso de `PARAMETROS_EDITAR`. Por ora, a sincronização é só na inicialização.
...
- Documento `5.01 - manter-usuario`: `../5.01 - manter-usuario/documento-analise-manter-usuario.md` (padrão do histórico via Envers — `/usuarios/historico/{id}`).
- Documento `5.02 - manter-perfil-permissao`: `../5.02 - manter-perfil-permissao/documento-analise-manter-perfil-permissao.md` (padrão "catálogo no código + sincronizador" espelhado aqui no catálogo de parâmetros).
